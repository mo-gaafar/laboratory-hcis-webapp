...
def viewconsumables():
    if request.method == 'GET':
        # Personal info
        mycursor.execute(
            "SELECT C.Name, Stock, SupplierContact, InventoryID FROM consumables AS C JOIN uses ON C.Name = Consumables_Name JOIN equipment ON EquipmentSerialNumber = Serial_Number")
